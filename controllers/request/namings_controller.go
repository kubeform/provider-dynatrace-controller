...
// NamingsReconciler reconciles a Namings object
type NamingsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=request.dynatrace.kubeform.com,resources=namings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=request.dynatrace.kubeform.com,resources=namings/status,verbs=get;update;patch

func (r *NamingsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("namings", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Namings")
...
func (r *NamingsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &requestv1alpha1.Namings{}); err != nil {
			klog.Error(err, "unable to set up auditor", requestv1alpha1.Namings{}.APIVersion, requestv1alpha1.Namings{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&requestv1alpha1.Namings{}).
