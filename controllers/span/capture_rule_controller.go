...
// CaptureRuleReconciler reconciles a CaptureRule object
type CaptureRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=span.dynatrace.kubeform.com,resources=capturerules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=span.dynatrace.kubeform.com,resources=capturerules/status,verbs=get;update;patch

func (r *CaptureRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("capturerule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CaptureRule")
...
func (r *CaptureRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spanv1alpha1.CaptureRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", spanv1alpha1.CaptureRule{}.APIVersion, spanv1alpha1.CaptureRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&spanv1alpha1.CaptureRule{}).
