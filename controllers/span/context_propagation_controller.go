...
// ContextPropagationReconciler reconciles a ContextPropagation object
type ContextPropagationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=span.dynatrace.kubeform.com,resources=contextpropagations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=span.dynatrace.kubeform.com,resources=contextpropagations/status,verbs=get;update;patch

func (r *ContextPropagationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("contextpropagation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContextPropagation")
...
func (r *ContextPropagationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spanv1alpha1.ContextPropagation{}); err != nil {
			klog.Error(err, "unable to set up auditor", spanv1alpha1.ContextPropagation{}.APIVersion, spanv1alpha1.ContextPropagation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&spanv1alpha1.ContextPropagation{}).
