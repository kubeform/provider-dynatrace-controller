...
// +kubebuilder:rbac:groups=azure.dynatrace.kubeform.com,resources=credentials,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=azure.dynatrace.kubeform.com,resources=credentials/status,verbs=get;update;patch

func (r *CredentialsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("credentials", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Credentials")
...
func (r *CredentialsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &azurev1alpha1.Credentials{}); err != nil {
			klog.Error(err, "unable to set up auditor", azurev1alpha1.Credentials{}.APIVersion, azurev1alpha1.Credentials{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&azurev1alpha1.Credentials{}).
