...
// RequestsReconciler reconciles a Requests object
type RequestsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=key.dynatrace.kubeform.com,resources=requests,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=key.dynatrace.kubeform.com,resources=requests/status,verbs=get;update;patch

func (r *RequestsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("requests", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Requests")
...
func (r *RequestsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &keyv1alpha1.Requests{}); err != nil {
			klog.Error(err, "unable to set up auditor", keyv1alpha1.Requests{}.APIVersion, keyv1alpha1.Requests{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&keyv1alpha1.Requests{}).
