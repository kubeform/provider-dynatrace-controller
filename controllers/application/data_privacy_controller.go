...
// DataPrivacyReconciler reconciles a DataPrivacy object
type DataPrivacyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=application.dynatrace.kubeform.com,resources=dataprivacies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=application.dynatrace.kubeform.com,resources=dataprivacies/status,verbs=get;update;patch

func (r *DataPrivacyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dataprivacy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DataPrivacy")
...
func (r *DataPrivacyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &applicationv1alpha1.DataPrivacy{}); err != nil {
			klog.Error(err, "unable to set up auditor", applicationv1alpha1.DataPrivacy{}.APIVersion, applicationv1alpha1.DataPrivacy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&applicationv1alpha1.DataPrivacy{}).
