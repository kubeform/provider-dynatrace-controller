...
// ErrorRulesReconciler reconciles a ErrorRules object
type ErrorRulesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=application.dynatrace.kubeform.com,resources=errorrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=application.dynatrace.kubeform.com,resources=errorrules/status,verbs=get;update;patch

func (r *ErrorRulesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("errorrules", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ErrorRules")
...
func (r *ErrorRulesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &applicationv1alpha1.ErrorRules{}); err != nil {
			klog.Error(err, "unable to set up auditor", applicationv1alpha1.ErrorRules{}.APIVersion, applicationv1alpha1.ErrorRules{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&applicationv1alpha1.ErrorRules{}).
