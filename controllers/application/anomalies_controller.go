...
// AnomaliesReconciler reconciles a Anomalies object
type AnomaliesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=application.dynatrace.kubeform.com,resources=anomalies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=application.dynatrace.kubeform.com,resources=anomalies/status,verbs=get;update;patch

func (r *AnomaliesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("anomalies", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Anomalies")
...
func (r *AnomaliesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &applicationv1alpha1.Anomalies{}); err != nil {
			klog.Error(err, "unable to set up auditor", applicationv1alpha1.Anomalies{}.APIVersion, applicationv1alpha1.Anomalies{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&applicationv1alpha1.Anomalies{}).
