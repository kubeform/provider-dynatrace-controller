...
// ServiceMetricReconciler reconciles a ServiceMetric object
type ServiceMetricReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=calculated.dynatrace.kubeform.com,resources=servicemetrics,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=calculated.dynatrace.kubeform.com,resources=servicemetrics/status,verbs=get;update;patch

func (r *ServiceMetricReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicemetric", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceMetric")
...
func (r *ServiceMetricReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &calculatedv1alpha1.ServiceMetric{}); err != nil {
			klog.Error(err, "unable to set up auditor", calculatedv1alpha1.ServiceMetric{}.APIVersion, calculatedv1alpha1.ServiceMetric{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&calculatedv1alpha1.ServiceMetric{}).
