...
// AutotagReconciler reconciles a Autotag object
type AutotagReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=autotag.dynatrace.kubeform.com,resources=autotags,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=autotag.dynatrace.kubeform.com,resources=autotags/status,verbs=get;update;patch

func (r *AutotagReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("autotag", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Autotag")
...
func (r *AutotagReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &autotagv1alpha1.Autotag{}); err != nil {
			klog.Error(err, "unable to set up auditor", autotagv1alpha1.Autotag{}.APIVersion, autotagv1alpha1.Autotag{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&autotagv1alpha1.Autotag{}).
