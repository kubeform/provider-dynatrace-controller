...
// SharingReconciler reconciles a Sharing object
type SharingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dashboard.dynatrace.kubeform.com,resources=sharings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dashboard.dynatrace.kubeform.com,resources=sharings/status,verbs=get;update;patch

func (r *SharingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sharing", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Sharing")
...
func (r *SharingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dashboardv1alpha1.Sharing{}); err != nil {
			klog.Error(err, "unable to set up auditor", dashboardv1alpha1.Sharing{}.APIVersion, dashboardv1alpha1.Sharing{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dashboardv1alpha1.Sharing{}).
