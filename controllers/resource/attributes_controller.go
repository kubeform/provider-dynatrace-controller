...
// AttributesReconciler reconciles a Attributes object
type AttributesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=resource.dynatrace.kubeform.com,resources=attributes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=resource.dynatrace.kubeform.com,resources=attributes/status,verbs=get;update;patch

func (r *AttributesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("attributes", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Attributes")
...
func (r *AttributesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &resourcev1alpha1.Attributes{}); err != nil {
			klog.Error(err, "unable to set up auditor", resourcev1alpha1.Attributes{}.APIVersion, resourcev1alpha1.Attributes{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&resourcev1alpha1.Attributes{}).
