...
// NamingReconciler reconciles a Naming object
type NamingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=processgroup.dynatrace.kubeform.com,resources=namings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=processgroup.dynatrace.kubeform.com,resources=namings/status,verbs=get;update;patch

func (r *NamingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("naming", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Naming")
...
func (r *NamingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &processgroupv1alpha1.Naming{}); err != nil {
			klog.Error(err, "unable to set up auditor", processgroupv1alpha1.Naming{}.APIVersion, processgroupv1alpha1.Naming{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&processgroupv1alpha1.Naming{}).
